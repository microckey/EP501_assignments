...
nuroich = 0;              %store the number of row interchange for determinant calculation
...
for ir1=1:n-1
    ...
    if (ipivmax ~= ir1)
        ...
        ord(ir1)=ord(ipivmax);
        ord(ipivmax)=itmp;
        nuroich = nuroich + 1; % update the number of row interchange
...
%% display the # of row interchanges
disp('The total number of row interchanges is:  ');
disp(nuroich)

%% compute & display the det.
detA = (-1)^nuroich;
Amododr = Amod(ord,:);
for k = 1 : n
    detA = detA*Amododr(k,k);
end %for
